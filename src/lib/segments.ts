 ...
 *   Addition = everyone below that (includes customers with no purchases yet)
 */

export type CustomerTier = "elite" | "addition";

/** Minimum lifetime spend (GH₵) to be an Elite customer. */
export const ELITE_MIN = 1200;

export const TIER_ORDER: CustomerTier[] = ["elite", "addition"];
...
export const TIER_BADGE: Record<CustomerTier, string> = {
  ...
  addition: "bg-zinc-100 text-zinc-700 dark:bg-zinc-800 dark:text-zinc-300",
};

export function tierForSpend(totalSpent: number): CustomerTier {
  return totalSpent >= ELITE_MIN ? "elite" : "addition";
}
